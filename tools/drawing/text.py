...
class DrawTextTool(BaseTool):
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        code = (
            f"const textObj = new fabric.Text('{escaped}', {{"
            f"left: {x}, top: {y}, "
            f"fontSize: {font_size}, fill: '{color}'"
            f"}});"
            f"canvas.add(textObj); canvas.renderAll();"
        )
        description = f"Drew text '{text}' at ({x}, {y}) with size {font_size}"
        return ToolResult(code=code, description=description, data={"type": "text", "x": x, "y": y, "text": text, "font_size": font_size, "color": color})
